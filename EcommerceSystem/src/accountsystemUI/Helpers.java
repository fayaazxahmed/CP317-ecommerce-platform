...
public class Helpers {
        ...
        if (file.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                ...
                if (!line.toLowerCase().startsWith("username")) {
                    ...
                    if (parts.length == 4) users.add(new User(parts[0].trim(), parts[1].trim(), parts[2].trim(), parts[3].trim()));
                }
    ...
    public static void saveUsers(List<User> users) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file))) {
            // Write header
            writer.write("Username,Password,Email,Address");
            writer.newLine();

            // Write users
            for (User user : users) {
                writer.write(user.getUsername() + "," + user.getPassword() + "," + user.getEmail() + "," + user.getAddress());
                writer.newLine();
            }
        } catch (IOException e) {
